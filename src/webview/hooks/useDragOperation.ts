...
export function useDragOperation(): DragOperationHook {
	...
	const _commits = useGitStore((state) => state.commits);
	 ...
	 */
	const isDangerousOperation = useCallback(
		(operation: GitOperation): boolean => {
...
function createRebaseOperation(
	source: {
		type: 'commit';
		commit: { hash: string; shortHash: string; message: string };
		branchName?: string;
	},
...
function createCherryPickOperation(
	...
	const needsCheckout =
		!target.branch.isHead && target.branch.name !== currentBranch;
...
function createMoveBranchOperation(
	source: {
		type: 'branch-pointer';
		branch: { name: string; isRemote: boolean };
		commitHash: string;
	},
	target: { type: 'commit'; commit: { hash: string; shortHash: string } },
): OperationInfo {
...
function createCreateBranchOperation(source: {
	type: 'commit';
	commit: { hash: string; shortHash: string };
}): OperationInfo {
	return {
