...
export type MessageDispatcher = {
  // Repository
  onRepoInfo?: (payload: Extract<ResponseMessage, { type: 'repo/info' }>['payload']) => void;
  onRepoStatus?: (payload: Extract<ResponseMessage, { type: 'repo/status' }>['payload']) => void;

  // Git
  onGitLog?: (payload: Extract<ResponseMessage, { type: 'git/log' }>['payload']) => void;
  onGitBranches?: (payload: Extract<ResponseMessage, { type: 'git/branches' }>['payload']) => void;
  onGitStashes?: (payload: Extract<ResponseMessage, { type: 'git/stashes' }>['payload']) => void;
  onGitSuccess?: (payload: Extract<ResponseMessage, { type: 'git/success' }>['payload']) => void;

  // Events
  onGitChanged?: () => void;
  onGitConflict?: (payload: Extract<EventMessage, { type: 'git/conflict' }>['payload']) => void;

  // Azure
  onAzurePRs?: (payload: Extract<ResponseMessage, { type: 'azure/prs' }>['payload']) => void;
  onAzureWorkItems?: (
    payload: Extract<ResponseMessage, { type: 'azure/workItems' }>['payload']
  ) => void;
  onAzurePipelineStatus?: (
    payload: Extract<ResponseMessage, { type: 'azure/pipelineStatus' }>['payload']
  ) => void;
  onAzurePolicyConfigurations?: (
    payload: Extract<ResponseMessage, { type: 'azure/policyConfigurations' }>['payload']
  ) => void;
  onAzurePolicyEvaluations?: (
    payload: Extract<ResponseMessage, { type: 'azure/policyEvaluations' }>['payload']
  ) => void;
  onAzureBuildDetails?: (
    payload: Extract<ResponseMessage, { type: 'azure/buildDetails' }>['payload']
  ) => void;
  onAzureTestResults?: (
    payload: Extract<ResponseMessage, { type: 'azure/testResults' }>['payload']
  ) => void;
  onAzureCodeCoverage?: (
    payload: Extract<ResponseMessage, { type: 'azure/codeCoverage' }>['payload']
  ) => void;
  onAzureRebuildTriggered?: (
    payload: Extract<ResponseMessage, { type: 'azure/rebuildTriggered' }>['payload']
  ) => void;
  onAzurePRCreated?: (
    payload: Extract<ResponseMessage, { type: 'azure/prCreated' }>['payload']
  ) => void;
  onAzureWorkItemLinked?: (
    payload: Extract<ResponseMessage, { type: 'azure/workItemLinked' }>['payload']
  ) => void;

  // GitHub
  onGitHubPRs?: (payload: Extract<ResponseMessage, { type: 'github/prs' }>['payload']) => void;
  ...
  const handleMessage = useCallback(
    (event: MessageEvent<ExtensionMessage>) => {
      ...
      switch (message.type) {
        ...
        default:
          if (debug) {
            console.warn('[useMessageHandler] Unknown message type:', (message as { type: string }).type);
          }
      }
    },
    [debug, onMessage]
...
export function combineDispatchers(...dispatchers: MessageDispatcher[]): MessageDispatcher {
  ...
  for (const key of dispatcherKeys) {
    ...
    if (handlers.length > 0) {
      ...
      (combined as Record<string, (...args: unknown[]) => void>)[key] = (...args: unknown[]) => {
