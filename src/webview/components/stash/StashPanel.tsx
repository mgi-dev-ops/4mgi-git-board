...
import { CreateStashDialog, CreateStashOptions } from "./CreateStashDialog";
import { StashContextMenu } from "./StashContextMenu";
import { StashDiffView, StashFileChange } from "./StashDiffView";
...
export function StashPanel({ variant = "panel", className = "" }: StashPanelProps) {
  // Store
  ...
  const handleViewStash = useCallback((stash: GitStash) => {
  ...
  const handleSelectDiffFile = useCallback((file: StashFileChange) => {
    setDiffView((prev) => {
      if (prev.stash) {
    ...
    return (
      <div className={`${styles.stashDropdown} ${className}`} ref={dropdownRef}>
        ...
        >
          ...
          <span>Stash</span>
          {stashes.length > 0 && (
            <span className={styles.dropdownTriggerBadge}>{stashes.length}</span>
          )}
