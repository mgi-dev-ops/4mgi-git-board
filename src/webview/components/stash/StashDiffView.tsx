...
import type { GitFileChange, GitStash } from '../../stores/gitStore';
		...
		return (
			<div className={styles.emptyState}>
				...
				<p className={styles.emptyDescription}>
					This stash contains no file changes.
				</p>
			</div>
		);
	}
	...
	return (
		...
		>
			{files.map((file) => (
				...
				>
					<span
						className={`${styles.diffFileStatus} ${getStatusClassName(file.status)}`}
					>
...
export function StashDiffView({
	...
	const [view, setView] = useState<'files' | 'diff'>(
				...
				{loading ? (
					...
					<>
						{selectedFile && (
							...
							>
								<span
									className={`${styles.diffFileStatus} ${getStatusClassName(selectedFile.status)}`}
								>
