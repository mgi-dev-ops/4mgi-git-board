...
export function parseCommitType(message: string): ParsedCommit {
	...
	if (match) {
		...
		const type = (
			COMMIT_TYPE_CONFIG[typeStr as CommitType] ? typeStr : 'other'
		) as CommitType;
	...
	if (prefixMatch) {
		...
		const type = (
			COMMIT_TYPE_CONFIG[prefix.toLowerCase() as CommitType]
				? prefix.toLowerCase()
				: 'other'
		) as CommitType;
 ...
 */
export function generateBranchColorMap(
	branches: GitBranch[],
): Map<string, string> {
	...
	for (let i = 0; i < commits.length; i++) {
		...
		if (isBoring && isLinear) {
			...
			if (consecutiveCount > maxConsecutive) {
				...
				for (
					let j = consecutiveStart;
					j < consecutiveStart + consecutiveCount;
					j++
				) {
