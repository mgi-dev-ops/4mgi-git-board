...
function BranchRef({ name, isRemote }: BranchRefProps) {
	...
	return (
		<span
			className={`${styles.branchRef} ${isOrigin ? styles.remote : styles.local}`}
		>
			[{displayName}]
		</span>
	);
...
function BuildStatusBadge({ status }: BuildStatusBadgeProps) {
	...
	return (
		<span
			className={`${styles.buildStatus} ${config.className}`}
			title={`Build: ${status}`}
		>
			{config.icon}
		</span>
...
function WorkItemBadge({ workItem }: WorkItemBadgeProps) {
	...
	return (
		<span
			className={styles.workItemBadge}
			title={workItem.title || `Work Item #${workItem.id}`}
		>
...
export function CommitNode({
	...
	const typeConfig = useMemo(
		() => getCommitTypeConfig(commit.parsedType.type),
			...
			{!compact && commit.branchRefs.length > 0 && (
				<span className={styles.refs}>
					...
					{commit.branchRefs.length > 2 && (
						<span className={styles.moreRefs}>
							+{commit.branchRefs.length - 2}
						</span>
					)}
				</span>
