  ...
  return (
    <div className={styles.sidebar}>
      ...
      <CollapsibleSection title="Branches" count={localBranches.length}>
        <ul className={styles.branchList} role="tree" aria-label="Local branches">
          {localBranches.length > 0 ? (
            localBranches.map((branch) => (
              ...
              >
                ...
                {branch.isProtected && (
                  <span className={styles.protectedBadge} title="Protected branch">
                    🔒
                  </span>
                )}
                {(branch.ahead !== undefined && branch.ahead > 0) && (
                  <span className={styles.aheadBehind} title={`${branch.ahead} ahead`}>
                    ↑{branch.ahead}
                  </span>
                )}
                {(branch.behind !== undefined && branch.behind > 0) && (
                  <span className={styles.aheadBehind} title={`${branch.behind} behind`}>
                    ↓{branch.behind}
                  </span>
                )}
              </li>
      ...
      >
        <ul className={styles.branchList} role="tree" aria-label="Remote branches">
      ...
      <CollapsibleSection title="Stashes" count={stashes.length}>
        <ul className={styles.stashList} role="list" aria-label="Stashes">
          {stashes.length > 0 ? (
            stashes.map((stash) => (
      ...
      <CollapsibleSection
        ...
        defaultExpanded={false}
      >
        <ul className={styles.workItemList} role="list" aria-label="Work items">
          {workItems.length > 0 ? (
            workItems.map((item) => (
              <li
                key={item.id}
