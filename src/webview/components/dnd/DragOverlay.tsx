      ...
      `.trim()}
		...
		>
          ...
          `.trim()}
			...
			{/* Drop hint */}
			{!operation && (
				<div className={styles.dragOverlayHint}>Drag to a target</div>
			)}
		</div>
	);
};
...
const BranchPreview: React.FC<BranchPreviewProps> = ({ source }) => {
	...
	return (
		<div className={styles.branchPreview}>
			...
			{branch.isRemote && <span className={styles.remoteBadge}>remote</span>}
		</div>
	);
};
...
const BranchPointerPreview: React.FC<BranchPointerPreviewProps> = ({
	source,
}) => {
...
const CherryPickIcon: React.FC = () => (
	<svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
		...
		<path
			d="M8 2V5M8 11V14M5 8H2M14 8H11"
			strokeWidth="1.5"
			stroke="currentColor"
			fill="none"
		/>
