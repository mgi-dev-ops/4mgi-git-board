  ...
  return (
    ...
    >
      ...
      {showHoverHighlight && hoverContent && (
        <div className={styles.dropZoneHoverContent}>
          {hoverContent}
        </div>
      )}
...
const DropFeedbackIndicator: React.FC<DropFeedbackIndicatorProps> = ({
  ...
  return (
    <div className={`${styles.feedbackIndicator} ${styles[`feedback--${state}`]}`}>
...
export interface EmptySpaceDropZoneProps {
  ...
  onCreateBranch?: (commitHash: string, position: { x: number; y: number }) => void;
}
