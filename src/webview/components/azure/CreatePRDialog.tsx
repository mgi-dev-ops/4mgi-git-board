...
import { WorkItemBadge, type WorkItemType } from './WorkItemBadge';
...
export const CreatePRDialog: React.FC<CreatePRDialogProps> = ({
	...
	const defaultTarget =
		targetBranches.find((b) => b.isDefault)?.name ||
		...
		>
			...
			>
				...
				<div className={styles.header}>
					<h2 id="create-pr-title" className={styles.title}>
						Create Pull Request
					</h2>
					<button
						type="button"
				...
				<form onSubmit={handleSubmit} className={styles.form}>
					...
					<div className={styles.branchSection}>
						<div className={styles.branchFlow}>
							...
							<div className={styles.branchBox}>
								<label htmlFor="targetBranch" className={styles.branchLabel}>
									Target
								</label>
								<select
									id="targetBranch"
					...
					{/* Description */}
					<div className={styles.field}>
						<label htmlFor="prDescription" className={styles.label}>
							Description
						</label>
						<textarea
							id="prDescription"
								...
								<div className={styles.selectedReviewers}>
									{selectedReviewers.map((reviewerId) => {
										...
										return (
											<span
												key={reviewerId}
												className={styles.selectedReviewer}
											>
												{reviewer.imageUrl ? (
													<img
					...
					{availableWorkItems.length > 0 && (
						<div className={styles.field}>
							<span className={styles.label}>Work Items to Link</span>
							<div className={styles.workItemsList}>
								{availableWorkItems.map((workItem) => (
									<label key={workItem.id} className={styles.workItemOption}>
										...
										<span className={styles.workItemTitle}>
											{workItem.title}
										</span>
									</label>
								))}
						...
						<label className={styles.checkboxLabel}>
							...
							<span className={styles.checkboxText}>
								...
								<span className={styles.checkboxHint}>
									Automatically complete this pull request when all policies are
									satisfied
								</span>
							</span>
						</label>
