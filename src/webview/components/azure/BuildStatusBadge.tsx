...
export type BuildStatus = "running" | "succeeded" | "failed" | "canceled" | "partial";
...
const statusConfig: Record<BuildStatus, { icon: React.ReactNode; label: string; ariaLabel: string }> = {
  running: {
    icon: (
...
export const BuildStatusBadge: React.FC<BuildStatusBadgeProps> = ({
  ...
  return (
    <span
      ...
      aria-label={buildNumber ? `${config.ariaLabel} #${buildNumber}` : config.ariaLabel}
      title={titleText}
    >
      ...
      {!compact && (
        <>
          {buildNumber && <span className={styles.buildNumber}>#{buildNumber}</span>}
