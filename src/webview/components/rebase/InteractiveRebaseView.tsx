...
import type { RebaseCommit, RebaseAction, RebaseState, ConflictInfo } from './types';
...
export function InteractiveRebaseView({
  ...
  const [rewordCommitIndex, setRewordCommitIndex] = useState<number | null>(null);
  ...
  const warningMessage = useMemo(() => {
    const nonDropCommits = commits.filter((c) => c.action !== 'drop').length;
  ...
  const moveCommit = useCallback((direction: 'up' | 'down') => {
  ...
  const handleRewordSave = useCallback((message: string) => {
  ...
  const handleSquashSave = useCallback((message: string) => {
  ...
  useEffect(() => {
    if (commitListRef.current) {
      const selectedRow = commitListRef.current.querySelector(`[data-index="${selectedIndex}"]`);
      if (selectedRow) {
