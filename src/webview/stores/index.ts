...
export function useCurrentBranch() {
	...
	return (
		branches.find((b) => b.name === currentBranchName && !b.isRemote) ?? null
	);
}
...
export function useCommitSelection() {
	...
	const multiSelectedCommits = useUIStore(
		(state) => state.multiSelectedCommits,
	);
	const selectCommit = useUIStore((state) => state.selectCommit);
	const toggleCommitSelection = useUIStore(
...
export function useFilterControls() {
	...
	const toggleShowMergeCommits = useUIStore(
