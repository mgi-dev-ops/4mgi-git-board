...
interface UIActions {
	...
	showContextMenu: (
		...
		target: { type: string; data: unknown },
	) => void;
	...
	devtools(
		persist(
			immer((set, get) => ({
				...
				selectCommitRange: (startHash, endHash, allHashes) =>
					set((state) => {
				...
				setDetailPanelHeight: (height) =>
					set((state) => {
						state.panels.detailPanelHeight = Math.max(
							100,
							Math.min(600, height),
						);
					}),

				setCommitListWidth: (width) =>
					set((state) => {
				...
				toggleBranchLabels: () =>
					set((state) => {
						state.graphSettings.showBranchLabels =
							!state.graphSettings.showBranchLabels;
					}),
				...
				toggleRemoteBranches: () =>
					set((state) => {
						state.graphSettings.showRemoteBranches =
							!state.graphSettings.showRemoteBranches;
					}),
				...
				// Context menu
				showContextMenu: (position, target) =>
					set((state) => {
...
export const selectIsDetailPanelOpen = (state: UIState) =>
	state.panels.detailPanelOpen;
