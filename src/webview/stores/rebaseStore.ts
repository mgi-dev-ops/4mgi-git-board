...
interface RebaseActions {
  // Plan management
  initRebasePlan: (commits: RebaseCommit[], onto: string, branch: string) => void;
  ...
  resolveConflictFile: (path: string, resolution: 'ours' | 'theirs' | 'merged', content?: string) => void;
...
function postMessage(type: string, payload?: Record<string, unknown>) {
  ...
  if (api) {
    ...
    console.warn('[RebaseStore] VS Code API not available, message not sent:', type);
  }
}
...
export const useRebaseStore = create<RebaseState & RebaseActions>()(
  devtools(
    immer((set, get) => ({
      ...
      // Plan management
      initRebasePlan: (commits, onto, branch) => set((state) => {
      ...
      // Commit actions
      setCommitAction: (index, action) => set((state) => {
        if (state.rebasePlan && index >= 0 && index < state.rebasePlan.commits.length) {
          state.rebasePlan.commits[index].action = action;
        }
      }),

      setAllCommitsAction: (action) => set((state) => {
        if (state.rebasePlan) {
          state.rebasePlan.commits.forEach(c => { c.action = action; });
        }
      }),

      rewordCommit: (index, newMessage) => set((state) => {
        if (state.rebasePlan && index >= 0 && index < state.rebasePlan.commits.length) {
      ...
      // Reorder commits
      moveCommit: (fromIndex, toIndex) => set((state) => {
      ...
      swapCommits: (index1, index2) => set((state) => {
        if (!state.rebasePlan) return;
        const commits = state.rebasePlan.commits;
        if (index1 < 0 || index1 >= commits.length) return;
        if (index2 < 0 || index2 >= commits.length) return;

        [commits[index1], commits[index2]] = [commits[index2], commits[index1]];
      }),
      ...
      startRebase: () => {
        ...
        if (!state.canStartRebase()) {
          set((s) => { s.error = 'Cannot start rebase: invalid plan or already rebasing'; });
          return;
        }
        set((s) => {
          s.loading = true;
      ...
      continueRebase: () => {
        ...
        if (!state.canContinueRebase()) {
          set((s) => { s.error = 'Cannot continue rebase: unresolved conflicts'; });
          return;
        }
        set((s) => {
          s.loading = true;
      ...
      setStage: (stage) => set((state) => {
        state.stage = stage;
        if (stage === 'idle' || stage === 'completed' || stage === 'aborted') {
          state.isRebasing = false;
        }
      }),

      setIsRebasing: (isRebasing) => set((state) => {
      ...
      markCommitProcessed: (hash) => set((state) => {
      ...
      resolveConflictFile: (path, resolution, content) => set((state) => {
        ...
        state.hasUnresolvedConflicts = state.conflicts.files.some(f => f.status === 'unresolved');
      }),

      markFileResolved: (path) => set((state) => {
        if (!state.conflicts) return;
        const file = state.conflicts.files.find(f => f.path === path);
        if (file) {
      ...
      markAllFilesResolved: () => set((state) => {
        if (!state.conflicts) return;
        state.conflicts.files.forEach(f => { f.status = 'resolved'; });
        state.hasUnresolvedConflicts = false;
      }),

      checkUnresolvedConflicts: () => {
        const { conflicts } = get();
        return conflicts ? conflicts.files.some(f => f.status === 'unresolved') : false;
      },

      // Edit mode
      setEditingCommit: (hash) => set((state) => {
      ...
      setAmendedMessage: (message) => set((state) => {
      ...
      // UI
      setShowRebaseDialog: (show) => set((state) => {
        state.showRebaseDialog = show;
      }),

      selectCommit: (index) => set((state) => {
        state.selectedCommitIndex = index;
      }),

      setDraggedCommit: (index) => set((state) => {
      ...
      validatePlan: () => {
        ...
        for (const commit of rebasePlan.commits) {
          if (commit.action === 'pick' || commit.action === 'reword' || commit.action === 'edit') {
            hasPickBefore = true;
          } else if ((commit.action === 'squash' || commit.action === 'fixup') && !hasPickBefore) {
            errors.push('Squash/fixup must follow a pick, reword, or edit action');
            break;
          }
        }
      ...
      executeRebasePlan: (plan) => {
        ...
        postMessage('git.executeRebasePlan', { plan: plan as unknown as Record<string, unknown> });
      },
...
export const selectHasConflicts = (state: RebaseState) => state.hasUnresolvedConflicts;
export const selectCommitByIndex = (index: number) => (state: RebaseState) =>
  state.rebasePlan?.commits[index] ?? null;
export const selectShowRebaseDialog = (state: RebaseState) => state.showRebaseDialog;
