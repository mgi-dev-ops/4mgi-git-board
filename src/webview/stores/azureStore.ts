...
export type PullRequestVoteStatus =
	| 'approved'
...
function postMessage(type: string, payload?: Record<string, unknown>) {
	...
	if (api) {
		...
		console.warn(
			'[AzureStore] VS Code API not available, message not sent:',
			type,
		);
	}
}
...
export const useAzureStore = create<AzureState & AzureActions>()(
	devtools(
		immer((set, get) => ({
			...
			setWorkItemsLoading: (loading) =>
				set((state) => {
					state.workItemsLoading = loading;
				}),

			setWorkItemsError: (error) =>
				set((state) => {
			...
			setPolicyEvaluations: (prId, evaluations) =>
				set((state) => {
			...
			setBuildsLoading: (loading) =>
				set((state) => {
					state.buildsLoading = loading;
				}),

			// Commit-WorkItem Links
			setCommitWorkItemLinks: (links) =>
				set((state) => {
					state.commitWorkItemLinks = links;
				}),

			linkCommitToWorkItem: (commitHash, workItemId) =>
				set((state) => {
			...
			unlinkCommitFromWorkItem: (commitHash, workItemId) =>
				set((state) => {
...
export const selectSelectedPullRequest = (state: AzureState) =>
	state.pullRequests.find((pr) => pr.id === state.selectedPullRequestId) ??
	null;
...
export const selectPRsForBranch = (branch: string) => (state: AzureState) =>
	state.pullRequests.filter(
		(pr) => pr.sourceRefName === `refs/heads/${branch}`,
	);
export const selectPolicyEvaluationsForPR =
	(prId: number) => (state: AzureState) =>
