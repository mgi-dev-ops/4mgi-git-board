...
function postMessage(type: string, payload?: Record<string, unknown>) {
  ...
  if (api) {
    ...
    console.warn('[GitStore] VS Code API not available, message not sent:', type);
  }
}
...
export const useGitStore = create<GitState & GitActions>()(
  devtools(
    immer((set, get) => ({
      ...
      appendCommits: (commits) => set((state) => {
      ...
      // Pagination
      setHasMoreCommits: (hasMore) => set((state) => {
        state.hasMoreCommits = hasMore;
      }),

      setCommitOffset: (offset) => set((state) => {
        state.commitOffset = offset;
      }),

      incrementCommitOffset: (count) => set((state) => {
