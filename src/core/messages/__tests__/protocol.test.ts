...
describe('MessageProtocol', () => {
	...
	beforeEach(() => {
		...
		mockContext = {
			extensionContext:
				extensionContext as unknown as HandlerContext['extensionContext'],
		...
		protocol.setWebview(
			mockWebview as unknown as Parameters<typeof protocol.setWebview>[0],
		);
	});
	...
	describe('on / off', () => {
		it('should register handler', async () => {
			const handler = vi
				.fn()
				.mockResolvedValue({ type: 'test.response', payload: {} });

			protocol.on('git.getLog' as any, handler);
			await protocol.handleMessage({ type: 'git.getLog' } as any);

			expect(handler).toHaveBeenCalledWith({ type: 'git.getLog' }, mockContext);
		});
...
describe('Message Flow Integration', () => {
	...
	it('should handle complete request-response cycle', async () => {
		// Register handler
		protocol.registerHandler('git.getCommits', async (_msg) => ({
