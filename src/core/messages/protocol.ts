...
type HandlerMap = {
	[K in RequestMessage['type']]?: MessageHandler<
...
export class MessageProtocol {
	...
	async handleMessage(message: RequestMessage): Promise<void> {
		...
		if (!handler) {
			console.warn(
				`[MessageProtocol] No handler for message type: ${message.type}`,
			);
			this.sendError(
				'HANDLER_NOT_FOUND',
		...
		try {
			const response = await (handler as MessageHandler<typeof message>)(
				message,
				this.context,
			);
	...
	registerHandler(
		...
		(this.handlers as HandlerMap & Record<string, unknown>)[type] = async (
			message: unknown,
