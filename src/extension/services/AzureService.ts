...
import { BuildStatus as AzureBuildStatusEnum } from "azure-devops-node-api/interfaces/BuildInterfaces";
...
import {
  ...
  AzureBuild,
  AzureTestCaseResult,
  AzureCodeCoverageData,
} from "./AzureService.types";

import { AzureAuthProvider, AzureConnectionConfig } from "./AzureAuthProvider";
...
export class AzureService implements vscode.Disposable {
  ...
  async authenticate(pat: string): Promise<boolean> {
    ...
    const success = await this.authProvider.authenticate(pat, organization, project);
    if (success) {
   ...
   */
  private async initializeRepository(config: AzureConnectionConfig): Promise<void> {
  ...
  async getWorkItem(id: number): Promise<WorkItem> {
    ...
    return this.withRetry(async () => {
      const workItem = await this.workItemApi!.getWorkItem(id, undefined, undefined, undefined);
  ...
  async getWorkItems(ids: number[]): Promise<WorkItem[]> {
    ...
    return this.withRetry(async () => {
      const workItems = await this.workItemApi!.getWorkItems(ids, undefined, undefined, undefined);
   ...
   */
  async linkWorkItemToCommit(workItemId: number, commitSha: string): Promise<void> {
    this.ensureAuthenticated();
   ...
   */
  async getPolicyConfigurations(branch: string): Promise<PolicyConfiguration[]> {
    this.ensureAuthenticated();
    ...
    return this.withRetry(async () => {
      ...
      return policies
        .filter((p) => {
          const settings = p.settings as { scope?: Array<{ refName?: string }> };
   ...
   * Trigger a new build
   */
  async triggerBuild(definitionId: number, branch: string): Promise<BuildDetails> {
    this.ensureAuthenticated();
 ...
 */
export function createAzureService(authProvider: AzureAuthProvider): AzureService {
