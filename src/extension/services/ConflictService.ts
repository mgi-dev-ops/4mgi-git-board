 ...
 */
export type ConflictOperationType =
	| 'merge'
	| 'rebase'
	| 'cherryPick'
	| 'revert';
...
export class ConflictService {
	 ...
	 */
	async parseConflictedFile(
		relativePath: string,
	): Promise<ConflictedFile | null> {
	 ...
	 */
	private async getRebaseProgress(): Promise<
	...
	private async getCurrentBranch(): Promise<string | undefined> {
		try {
			const head = fs
				.readFileSync(path.join(this.gitDir, 'HEAD'), 'utf-8')
				.trim();
