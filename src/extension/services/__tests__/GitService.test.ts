...
import { GitService, createGitService, GitError, GitErrorCode } from '../GitService';
...
describe('GitService', () => {
  ...
  describe('getRepositoryInfo', () => {
    it('should return repository info for valid git repo', async () => {
      ...
      mockGit.raw.mockImplementation((args: string[]) => {
        if (args.includes('--is-shallow-repository')) return Promise.resolve('false');
        if (args.includes('--is-bare-repository')) return Promise.resolve('false');
  ...
  describe('getBranches', () => {
    it('should return local and remote branches', async () => {
      ...
      mockGit.branch.mockResolvedValue({
        ...
        branches: {
          'origin/main': { current: false, name: 'origin/main', commit: 'abc123', label: 'origin/main' },
          'origin/develop': { current: false, name: 'origin/develop', commit: 'def456', label: 'origin/develop' },
        },
        current: '',
      });
      mockGit.raw.mockResolvedValue('');
  ...
  describe('createBranch', () => {
    ...
    it('should create branch from specific commit', async () => {
      ...
      expect(mockGit.checkoutBranch).toHaveBeenCalledWith('feature/from-commit', 'abc123');
    });
  });
  ...
  describe('stash operations', () => {
    ...
    it('should create stash with message', async () => {
      ...
      expect(mockGit.raw).toHaveBeenCalledWith(['stash', 'push', '-m', 'My stash message']);
    });
  ...
  describe('remote operations', () => {
    ...
    it('should push to remote', async () => {
      ...
      await gitService.push('origin', 'main', { force: true, setUpstream: true });

      expect(mockGit.push).toHaveBeenCalledWith(['--force', '-u', 'origin', 'main']);
    });
  });
  ...
  describe('utility methods', () => {
    ...
    it('should remove lock file', () => {
      ...
      expect(fs.unlinkSync).toHaveBeenCalledWith(path.join(testWorkspace, '.git', 'index.lock'));
    });
  });
});
