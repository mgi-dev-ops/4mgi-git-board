...
import simpleGit, {
  ...
  LogResult,
  BranchSummary,
  StatusResult as SimpleGitStatus,
  DiffResult as SimpleGitDiff,
} from "simple-git";
...
export class GitService {
  ...
  async getRepositoryInfo(): Promise<RepositoryInfo> {
    try {
      ...
      const remotes: RemoteInfo[] = (remotesRaw as unknown as SimpleGitRemote[]).map(mapRemote);
      const hasSubmodules = fs.existsSync(path.join(this.workspacePath, ".gitmodules"));
  ...
  async getStatus(): Promise<StatusResult> {
    try {
      const status = (await this.git.status()) as unknown as SimpleGitStatusResult;
  ...
  async getLog(options: LogOptions = {}): Promise<Commit[]> {
    try {
      ...
      for (const entry of entries) {
        ...
        if (parts.length >= 8) {
          const [sha, shortSha, message, body, authorName, authorEmail, date, parentsStr, refs] = parts;
  ...
  async commit(
    ...
    try {
      ...
      const result = await this.git.commit(message, files, commitOptions as Record<string, string | number | boolean>);
  ...
  async getBranches(): Promise<Branch[]> {
    try {
      ...
      const trackingInfo = new Map<string, { ahead: number; behind: number; tracking?: string }>();
      ...
      for (const branchName of Object.keys(localBranches.branches)) {
        try {
          ...
          const [ahead, behind] = trackingResult.trim().split(/\s+/).map(Number);
