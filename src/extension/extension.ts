...
import { createGitService, GitService } from "./services/GitService";
import { AzureService, createAzureService } from "./services/AzureService";
...
function registerMessageHandlers(
  ...
  protocol.registerHandler("git.merge", async (payload) => {
    const result = await git.merge(payload.branch, { noFastForward: payload.noFf });
    return result;
  });
  ...
  protocol.registerHandler("git.getStashDiff", async (payload) => {
    ...
    const git_instance = git as any;
    const diff = await git_instance.git.raw([
      "stash",
      "show",
      "-p",
      `stash@{${payload.index}}`,
  ...
  protocol.registerHandler("git.getStashFiles", async (payload) => {
    const git_instance = git as any;
    const result = await git_instance.git.raw([
      "stash",
      "show",
      "--name-status",
      `stash@{${payload.index}}`,
  ...
  protocol.registerHandler("git.rebaseViaDrag", async (payload) => {
    ...
    // Save state for undo
    const currentBranch = sourceBranch || (await git.getBranches()).find(b => b.isCurrent)?.name;
    pushUndoOperation({
      type: "rebase",
  ...
  protocol.registerHandler("git.startRebase", async (payload) => {
    ...
    // Save state for undo
    const currentBranch = (await git.getBranches()).find(b => b.isCurrent)?.name;
    pushUndoOperation({
      type: "rebase",
      data: { branch: currentBranch, originalHead: await getHeadCommit(git) },
      timestamp: Date.now(),
    ...
    // Create rebase-todo content from plan
    const todoContent = plan.commits
      .map((c: { action: string; hash: string; message: string }) =>
        `${c.action} ${c.hash} ${c.message}`
      )
      .join("\n");
    ...
    try {
      ...
      if (plan.onto) {
        await git_instance.git.raw(["rebase", "--onto", plan.onto, plan.upstream || `${plan.onto}~${plan.commits.length}`]);
      } else {
        await git_instance.git.raw(["rebase", "-i", `HEAD~${plan.commits.length}`]);
      }
      ...
      if (state === "rebasing") {
        return { success: false, stage: "conflict", message: "Rebase conflict detected" };
      }
      throw error;
    }
  });
  ...
  protocol.registerHandler("git.skipRebaseCommit", async () => {
    const git_instance = git as any;
    await git_instance.git.raw(["rebase", "--skip"]);
    const state = git.getRepositoryState();
    return {
      success: state !== "rebasing",
  ...
  protocol.registerHandler("git.cherryPickViaDrag", async (payload) => {
    ...
    try {
      ...
      if (state === "cherry-picking") {
        return { success: false, stage: "conflict", message: "Cherry-pick conflict detected" };
      }
      throw error;
    }
  });
  ...
  protocol.registerHandler("git.abortCherryPick", async () => {
    const git_instance = git as any;
    await git_instance.git.raw(["cherry-pick", "--abort"]);
    return { success: true };
  });
  ...
  protocol.registerHandler("git.continueCherryPick", async () => {
    const git_instance = git as any;
    await git_instance.git.raw(["cherry-pick", "--continue"]);
    return { success: true };
  });
    ...
    try {
      switch (operation.type) {
        case "commit":
          ...
          await git_instance.git.raw(["reset", "--soft", "HEAD~1"]);
          break;
        case "rebase":
        case "cherryPick":
        case "merge":
          ...
          if (operation.data.originalHead) {
            await git_instance.git.raw(["reset", "--hard", operation.data.originalHead]);
          }
          break;
        case "checkout":
        ...
        case "stash":
          ...
          break;
        default:
          return { success: false, message: `Cannot undo operation: ${operation.type}` };
      }
    ...
    try {
      switch (operation.type) {
        ...
        case "rebase":
          ...
          return { success: false, message: "Cannot redo rebase automatically" };
        case "checkout":
        ...
        case "stash":
          ...
          break;
        default:
          return { success: false, message: `Cannot redo operation: ${operation.type}` };
      }
  ...
  protocol.registerHandler("git.getUndoRedoState", async () => {
    return {
      ...
      undoStack: undoStack.map(op => ({ type: op.type, timestamp: op.timestamp })),
      redoStack: redoStack.map(op => ({ type: op.type, timestamp: op.timestamp })),
    };
  });
...
async function getHeadCommit(git: GitService): Promise<string> {
  const git_instance = git as any;
  const result = await git_instance.git.raw(["rev-parse", "HEAD"]);
  return result.trim();
}
